...
if (!is_file(ROOT_PATH . '/config/database.php')) {
    http_response_code(503);
    header('Content-Type: text/plain; charset=utf-8');
    exit('El sistema aún no tiene configurada la conexión a la base de datos.');
}
...
if (file_exists(ROOT_PATH . '/vendor/autoload.php')) {
    ...
    http_response_code(503);
    header('Content-Type: text/plain; charset=utf-8');
    exit('Faltan las dependencias del sistema.');
}
...
session_set_cookie_params([
    ...
    'path'     => defined('BASE_URL_PATH') ? BASE_URL_PATH : '/',
    'secure'   => $secureCookie,
    'httponly' => true,
    'samesite' => 'Lax',
]);
