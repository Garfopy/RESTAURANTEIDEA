<?php
/**
 * Vista: Facturas CFDI de la empresa
 * Acceso: admin_empresa, supervisor
 */
...
<!-- ── Banner informativo (solo si no hay token configurado) ─────────────────── -->
<?php
$db     = Database::getInstance();
$stTok  = $db->prepare("SELECT valor FROM global_settings WHERE clave = 'facturalo_token' LIMIT 1");
$stTok->execute();
...
<div style="background:#FEF3C7;border:1px solid #F59E0B;border-radius:12px;padding:16px 20px;margin-bottom:20px;display:flex;align-items:flex-start;gap:12px">
  ...
  <div>
    ...
    <p style="font-size:.82rem;color:#92400E">
      Para generar facturas oficiales (CFDI) el superadmin debe configurar el token de
      <strong>factura-lo.mx</strong> en <em>Configuración → APIs y servicios</em>.<br>
      Es gratuito para empezar y no requiere instalar nada.
    </p>
  </div>
</div>
<?php endif; ?>
...
<div style="background:#fff;border-radius:12px;border:1px solid #E5E7EB;padding:20px;margin-bottom:20px">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
    <h2 style="font-size:.9rem;font-weight:700;color:#111827;margin:0">
      Pedidos entregados pendientes de facturar
      <span style="background:#EF4444;color:#fff;font-size:.65rem;padding:2px 8px;border-radius:999px;margin-left:8px;font-weight:700"><?= count($pedidosSinFactura) ?></span>
    </h2>
  </div>
...
<div style="background:#fff;border-radius:12px;border:1px solid #E5E7EB;overflow:hidden">
  ...
  <div style="padding:48px 20px;text-align:center">
    ...
    <p style="color:#D1D5DB;font-size:.8rem;margin-top:4px">Las facturas generadas para pedidos entregados aparecerán aquí</p>
  </div>
  <?php else: ?>
  <div style="overflow-x:auto">
    <table style="width:100%;border-collapse:collapse;font-size:.845rem">
      <thead>
        <tr style="border-bottom:2px solid #F3F4F6;background:#FAFAFA">
          ...
          <th style="text-align:left;padding:10px 12px;color:#6B7280;font-weight:600">Fecha emisión</th>
          <th style="text-align:right;padding:10px 12px;color:#6B7280;font-weight:600">Total</th>
          ...
          <?php if ($esAdmin): ?>
          <th style="text-align:center;padding:10px 12px;color:#6B7280;font-weight:600"></th>
          <?php endif; ?>
        </tr>
      </thead>
      <tbody>
        <?php foreach ($facturas as $f):
          $esCancelada = $f['estado'] === 'cancelada';
        ?>
        <tr style="border-bottom:1px solid #F9FAFB;<?= $esCancelada ? 'opacity:.6' : '' ?>">
          <td style="padding:10px 16px;font-family:monospace;font-size:.78rem;color:#374151">
            <?= htmlspecialchars($f['uuid_cfdi']) ?>
          </td>
          <td style="padding:10px 12px;font-weight:600;color:#111827">
            <?= htmlspecialchars($f['pedido_folio']) ?>
          </td>
          <td style="padding:10px 12px;color:#6B7280;white-space:nowrap">
            <?= $fecha($f['fecha_emision']) ?>
          </td>
          <td style="padding:10px 12px;text-align:right;font-weight:700;color:#111827">
            <?= $fmt($f['total']) ?>
          </td>
          ...
          <td style="padding:10px 16px;text-align:center">
            <div style="display:flex;align-items:center;justify-content:center;gap:8px">
              ...
              <a href="<?= htmlspecialchars($f['pdf_url']) ?>" target="_blank" rel="noopener"
                 title="Descargar PDF"
                 style="display:inline-flex;align-items:center;gap:5px;padding:5px 12px;background:#EF4444;color:#fff;border-radius:6px;font-size:.75rem;font-weight:600;text-decoration:none">
                <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                PDF
              </a>
              <?php endif; ?>
              <?php if (!empty($f['xml_url'])): ?>
              <a href="<?= htmlspecialchars($f['xml_url']) ?>" target="_blank" rel="noopener"
                 title="Descargar XML"
                 style="display:inline-flex;align-items:center;gap:5px;padding:5px 12px;background:#2563EB;color:#fff;border-radius:6px;font-size:.75rem;font-weight:600;text-decoration:none">
                <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"/></svg>
                XML
              </a>
              <?php endif; ?>
            </div>
          ...
          <td style="padding:10px 12px;text-align:center">
            ...
            <a href="<?= BASE_URL ?>empresa-factura/cancelar/<?= urlencode($f['uuid_cfdi']) ?>"
               onclick="return confirm('¿Cancelar esta factura ante el SAT? Esta acción no se puede deshacer.')"
  ...
  <!-- Paginación -->
  <?php if ($totalPages > 1): ?>
  <div style="padding:14px 20px;border-top:1px solid #F3F4F6;display:flex;align-items:center;gap:8px">
    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
      <a href="?page=<?= $i ?>"
         style="padding:6px 12px;border-radius:6px;font-size:.8rem;font-weight:600;text-decoration:none;
                <?= $i === $page ? 'background:var(--color-primary);color:#fff' : 'background:#F3F4F6;color:#374151' ?>">
        <?= $i ?>
      </a>
    <?php endfor; ?>
  </div>
...
<details style="margin-top:20px">
  <summary style="cursor:pointer;font-size:.8rem;font-weight:600;color:#6B7280;padding:10px 0;list-style:none;display:flex;align-items:center;gap:6px">
  ...
  <div style="background:#F9FAFB;border-radius:10px;padding:16px 20px;margin-top:8px;font-size:.83rem;color:#374151;line-height:1.7">
    <p><strong>¿Qué es un CFDI?</strong> Es la factura electrónica oficial de México, avalada por el SAT. Tiene un UUID único, un archivo XML y un PDF que puedes descargar y entregar a tu contador.</p>
    <p style="margin-top:8px"><strong>¿Quién emite la factura?</strong> CarniHub emite la factura <em>hacia tu empresa</em> por los pedidos que realizas. Es como cuando compras y pides tu factura al negocio.</p>
    <p style="margin-top:8px"><strong>¿Qué necesitas?</strong> Tu empresa ya debe tener RFC y régimen fiscal registrados. El superadmin de la plataforma configura las credenciales de timbrado.</p>
    <p style="margin-top:8px"><strong>¿Cuándo puedo facturar?</strong> Solo pedidos con estado <em>Entregado</em> pueden facturarse.</p>
  </div>
</details>
