...
carritoInicial.forEach(item => {
  if (parseFloat(item.cantidad) > 0) {
    preciosProductos[item.producto_id] = {
      ...
      precioBase: parseFloat(item.precio),
      nombre: item.nombre,
      ...
      .then(d => {
        ...
        if (precioEl) {
          if (precio < precioBase) {
            precioEl.innerHTML = '<span style="text-decoration:line-through;color:#9CA3AF;font-size:.75rem">$' + precioBase.toLocaleString('es-MX', {minimumFractionDigits:2}) + '</span> <span style="color:#059669;font-weight:700">$' + precio.toLocaleString('es-MX', {minimumFractionDigits:2}) + '</span>';
          } else {
