...
<head>
  ...
  <style>
    ...
    .access-switch {
      display:block;text-align:center;margin-top:14px;padding:10px 12px;
      background:#F9FAFB;border:1px dashed #D1D5DB;border-radius:10px;
      font-size:.82rem;color:#6B7280;text-decoration:none;font-weight:500;
    }
    .access-switch:hover { background:#F3F4F6;color:#374151; }
    .access-switch strong { color:var(--cp); }
  </style>
</head>
<body>
<?php
  ...
  <div class="staff-login-card">
    ...
    <div style="text-align:center;margin-bottom:22px">
      ...
      <div style="font-size:.78rem;color:#9CA3AF;margin-top:3px">
        <?= $modo === 'staff' ? 'Portal de acceso staff' : 'Identifícate para ordenar' ?>
      </div>
    </div>
    ...
    <a class="access-switch" href="<?= BASE_URL ?>acceso/<?= htmlspecialchars($slug ?? '') ?>">
      🍽 ¿Eres <strong>cliente</strong>? Entra al menú aquí
    </a>
    <?php endif; ?>
    <a class="access-switch" href="<?= BASE_URL ?>menu/<?= htmlspecialchars($slug ?? '') ?>"
       style="background:transparent;border:none;color:#9CA3AF;margin-top:6px;padding:6px">
      Ver el menú sin iniciar sesión →
    </a>
    <?php endif; ?>

    <?php endif; // restaurante existe ?>
