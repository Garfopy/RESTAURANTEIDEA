<?php
$_cfgReset = new ConfigModel();
$_appLogo  = $_cfgReset->get('app_logo', '');
$_appName  = $_cfgReset->get('app_name', APP_NAME);
...
<head>
  ...
  <title><?= htmlspecialchars($_appName) ?> — Nueva contraseña</title>
  ...
  <style>
    ...
    .login-glow-btm {
      ...
      filter: blur(55px); animation: glowPulse 4s ease-in-out infinite reverse;
      pointer-events: none; z-index: 0;
    }
    .login-accent-bar {
      width: 44px; height: 4px; border-radius: 2px;
      background: linear-gradient(90deg, #C8102E, #FF2E52);
      margin-bottom: 18px; box-shadow: 0 2px 12px rgba(200,16,46,.5);
    }
    .login-tagline {
    ...
    .input-login:focus { border-color: #C8102E; box-shadow: 0 0 0 3px rgba(200,16,46,.11); }
    ...
    .pw-toggle-btn {
      ...
      color: #9CA3AF; transition: color .2s; width: 22px; height: 22px;
    }
    ...
    .btn-login-submit {
      ...
      color: #fff; background: linear-gradient(135deg, #C8102E 0%, #A00D24 100%);
    ...
    .pw-hint {
      font-size: .75rem; color: #9CA3AF; margin-top: -8px; margin-bottom: 16px;
    }
...
<div class="login-card-wrap">
  ...
  <div class="login-left hide-mobile">
    ...
    <div style="position:relative;z-index:1;width:100%;display:flex;flex-direction:column;align-items:center">
      ...
      <p style="color:#94A3B8;text-align:center;font-size:.84rem;line-height:1.65;max-width:240px;margin:0 0 28px">
        Elige una contraseña segura que no hayas usado antes.
      </p>

      <!-- Tips de contraseña -->
      <div style="width:100%;position:relative;z-index:1">
        <p style="font-size:.72rem;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:12px">
        ...
        <?php
          ...
          foreach ($tips as $tip): ?>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;font-size:.82rem;color:#94A3B8">
          <div style="width:6px;height:6px;border-radius:50%;background:#C8102E;flex-shrink:0;box-shadow:0 0 6px rgba(200,16,46,.6)"></div>
          <?= $tip ?>
        </div>
        <?php endforeach; ?>
      </div>
  ...
  <div class="login-right">
    <div style="width:100%;max-width:380px">
      ...
      <div style="margin-bottom:28px">
        <h1 style="font-size:1.5rem;font-weight:800;color:#111827;margin:0 0 6px">Nueva contraseña</h1>
      ...
      <form method="POST" action="<?= BASE_URL ?>auth/doReset">
        <input type="hidden" name="token" value="<?= $_token ?>">

        <div>
          <label class="form-label" style="display:block;margin-bottom:6px">Nueva contraseña</label>
          <div class="input-wrap pw-wrap" id="pwWrap1">
            ...
            <input type="password" name="password" id="pw1" class="input-login"
                   placeholder="Mínimo 8 caracteres" required autocomplete="new-password">
            <button type="button" onclick="togglePw('pw1','pwWrap1')" class="pw-toggle-btn" aria-label="Mostrar contraseña">
        ...
        <div>
          <label class="form-label" style="display:block;margin-bottom:6px">Confirmar contraseña</label>
          <div class="input-wrap pw-wrap" id="pwWrap2">
            <span class="input-icon-left">
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                <rect x="5" y="11" width="14" height="10" rx="2" ry="2"/>
                <path stroke-linecap="round" stroke-linejoin="round" d="M8 11V7a4 4 0 018 0v4"/>
            ...
            <input type="password" name="password_confirm" id="pw2" class="input-login"
                   placeholder="Repite la contraseña" required autocomplete="new-password">
            <button type="button" onclick="togglePw('pw2','pwWrap2')" class="pw-toggle-btn" aria-label="Mostrar contraseña">
              <svg class="icon-eye" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
              </svg>
        ...
        <button type="submit" class="btn-login-submit" style="margin-top:8px">Actualizar contraseña</button>
      </form>
...
<script>
function togglePw(inputId, wrapId) {
