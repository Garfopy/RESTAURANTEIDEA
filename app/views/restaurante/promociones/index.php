...
<div style="background:#F9FAFB;border:2px dashed #D1D5DB;border-radius:12px;padding:48px 24px;text-align:center">
  ...
  <table style="width:100%;border-collapse:collapse;font-size:.875rem">
    <thead>
      <tr style="background:#F9FAFB;border-bottom:1px solid #E5E7EB">
        ...
        <th style="padding:12px 16px;text-align:left;font-weight:600;color:#374151">Vigencia</th>
        <th style="padding:12px 16px;text-align:center;font-weight:600;color:#374151">Comensales</th>
    ...
    <tbody>
      ...
      <?php
        ...
        $tipoLabel = match($p['tipo'] ?? 'porcentaje') {
          'porcentaje'   => $p['valor_descuento'] . '% off',
          'monto_fijo'   => '$' . number_format($p['valor_descuento'], 2) . ' off',
      ...
      <tr style="border-bottom:1px solid #F3F4F6">
        <td style="padding:12px 16px">
          <div style="font-weight:600;color:#111827"><?= htmlspecialchars($p['titulo']) ?></div>
        ...
        <td style="padding:12px 16px;font-weight:500"><?= $tipoLabel ?></td>
        <td style="padding:12px 16px;font-size:.82rem;color:#374151">
          <?= date('d/m/Y', strtotime($p['fecha_inicio'])) ?> → <?= date('d/m/Y', strtotime($p['fecha_fin'])) ?>
        </td>
        <td style="padding:12px 16px;text-align:center">
          <span style="font-weight:600;color:#374151"><?= (int)($p['total_asignados'] ?? 0) ?></span>
          <?php if ((int)($p['total_usados'] ?? 0) > 0): ?>
          <span style="font-size:.75rem;color:#059669;display:block"><?= $p['total_usados'] ?> usado(s)</span>
          <?php endif; ?>
        </td>
        <td style="padding:12px 16px;text-align:center">
          <span style="display:inline-block;padding:4px 10px;border-radius:99px;font-size:.75rem;font-weight:600;
        ...
        <td style="padding:12px 16px;text-align:right;white-space:nowrap">
          <a href="<?= BASE_URL ?>rest-promocion/editar/<?= $p['id'] ?>"
             style="font-size:.82rem;color:var(--cp);font-weight:500;text-decoration:none;margin-right:12px">
            Editar
          </a>
          <form method="POST" action="<?= BASE_URL ?>rest-promocion/eliminar/<?= $p['id'] ?>"
                style="display:inline"
                onsubmit="return confirm('¿Eliminar esta promoción? Esta acción no se puede deshacer.')">
            <button type="submit"
                    style="background:none;border:none;color:#EF4444;font-size:.82rem;font-weight:500;cursor:pointer;padding:0">
              Eliminar
            </button>
          </form>
        </td>
