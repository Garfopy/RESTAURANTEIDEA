...
<div style="max-width:600px;margin:0 auto">
  ...
  <div style="background:#fff;border-radius:12px;border:1px solid #E5E7EB;padding:28px">
    <div style="text-align:center;margin-bottom:20px">
      ...
      <div style="color:#6B7280;font-size:.875rem"><?= htmlspecialchars($ticket['mesa_nombre'] ?? 'Sin mesa') ?></div>
    </div>
    ...
    <?php else: ?>
    <div style="padding:14px;background:#F9FAFB;border:1px dashed #D1D5DB;border-radius:10px;color:#6B7280;text-align:center;margin-bottom:16px">
      Este ticket no tiene items activos.
    </div>
    <?php endif; ?>
    ...
    <div style="border-top:1px solid #E5E7EB;padding-top:12px;margin-top:4px">
      <div style="display:flex;justify-content:space-between;padding:6px 0;color:#6B7280">
        <span>Subtotal</span>
        <span>$<?= number_format((float)$ticket['subtotal'], 2) ?></span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:6px 0;color:#6B7280">
        <span>Propina</span>
        <span style="color:#10B981">$<?= number_format((float)$ticket['propina'], 2) ?></span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:12px 0 0;font-size:1.1rem;font-weight:700;border-top:1px solid #F3F4F6;margin-top:4px">
        <span>Total</span>
        <span>$<?= number_format((float)$ticket['total'], 2) ?></span>
      </div>
    </div>
    ...
    <form method="POST" action="<?= BASE_URL ?>rest-ticket/confirmarPago/<?= $ticket['id'] ?>" style="margin-top:16px">
      <div style="margin-bottom:12px">
        <label style="font-size:.85rem;font-weight:500">Metodo de pago</label>
