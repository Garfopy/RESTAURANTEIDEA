...
{
    // GET /registro/index — elegir tipo
    ...
    public function guardar(?string $p = null): void
    {
        ...
        if (!$ubicacion)                                     $errores[] = 'Selecciona una ubicación en el mapa.';
        ...
        if ($tipo === 'comprador') {
            $stmt = $db->prepare("
                INSERT INTO empresas (razon_social, tipo_negocio, activo, created_at)
                VALUES (?, ?, 0, NOW())
            ");
            ...
            $stmt->execute([$empresaId, $userId]);
        }

        // Generar token de verificación (expira en 24 h)
        $token = bin2hex(random_bytes(32));
        $stmt = $db->prepare("
            INSERT INTO verificacion_tokens (usuario_id, token, tipo, expires_at)
            VALUES (?, ?, 'email_verificacion', DATE_ADD(NOW(), INTERVAL 24 HOUR))
        ");
        $stmt->execute([$userId, $token]);

        // Registrar intento de registro (rate limiting)
        $stmt = $db->prepare("INSERT INTO registro_intentos (ip, email) VALUES (?, ?)");
        $stmt->execute([$ip, $email]);

        // Enviar correo de verificación
        $this->enviarEmailVerificacion($email, $nombre, $token);

        $this->flash('success', 'Cuenta creada exitosamente. Revisa tu correo (' . htmlspecialchars($email) . ') y haz clic en el enlace de verificación.');
    ...
    public function verificar(?string $token = null): void
    {
        ...
        $db   = Database::getInstance();
        $stmt = $db->prepare("
            SELECT vt.id, vt.usuario_id, u.nombre
        ...
        // Activar usuario y marcar token como usado
        ...
        $this->flash('success', '¡Correo verificado, ' . htmlspecialchars($row['nombre']) . '! Ya puedes iniciar sesión. Tu cuenta será activada en breve por un administrador.');
        $this->redirect('auth/login');
    }

    // GET /registro/pendiente — página informativa tras registrarse
