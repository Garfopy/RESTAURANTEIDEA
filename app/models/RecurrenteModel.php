...
�sticas de patrones de compra frecuente basadas en el historial
 * real de pedidos (pedidos + pedido_detalle + productos + usuarios).
 */
class RecurrenteModel extends BaseModel
{
    ...
    /** Resumen general: pedidos totales, compradores �nicos, productos distintos */
    public function getResumen(int $empresaId): array
    {
        $row = $this->queryOne(
            "SELECT
    ...
    /** Top productos m�s pedidos por n�mero de pedidos y cantidad acumulada */
    public function getTopProductos(int $empresaId, int $limit = 10): array
    {
        return $this->query(
            "SELECT
    ...
    /** Pedidos agrupados por d�a de la semana (todos los d�as, incluso con 0) */
    public function getPedidosPorDiaSemana(int $empresaId): array
    {
        $nombres = ['Domingo', 'Lunes', 'Martes', 'Mi�rcoles', 'Jueves', 'Viernes', 'S�bado'];
        $rows    = $this->query(
            "SELECT DAYOFWEEK(created_at) AS dia_num, COUNT(*) AS total_pedidos
             FROM pedidos
             WHERE empresa_id = ? AND estado != 'cancelado'
    ...
    /** Top compradores m�s frecuentes */
    public function getTopCompradores(int $empresaId, int $limit = 8): array
    {
        return $this->query(
            "SELECT
    ...
    /** Productos pedidos repetidamente por el mismo comprador (patr�n recurrente real) */
    public function getProductosRecurrentes(int $empresaId, int $minVeces = 2, int $limit = 15): array
    {
        return $this->query(
            "SELECT
