...
- [x] **Eliminar modo social por completo** — **HECHO**. Sprint 1 (2026-08-25) quitó `RestModeracionController` + vistas. El 2026-08-26 se remató el resto: `app/models/RestSocialModeracionModel.php` (990 líneas, era código muerto: consultaba `social_reports`/`social_blocks`/`social_photo_moderation`, tablas que ya no existen), rutas API `/admin/social/*` (`ApiController::social()`, `adminSocialRouter()`, el `case 'social'` del router admin), su routing y `publicPath` en `index.php`, el método de pago `social_cover` del dashboard financiero, y la tabla huérfana `moderation_actions` (`migrations/005_eliminar_modo_social.sql`). Verificado con grep: no queda ninguna referencia a social en el código (solo `razon_social`, que es el campo fiscal y no tiene relación)
